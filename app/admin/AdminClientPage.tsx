...
export default function AdminClientPage({ tenantId }: AdminPageProps) {
    ...
    const handleAddPage = async () => {
        ...
        const res = await createPage(name, slug, tenantId);
        if (res.success) {
            await loadPages();
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ padding: '0 1rem', marginBottom: '1rem' }}>
                    <div style={{ fontSize: '0.7rem', color: '#6b7280', marginBottom: '0.5rem', fontWeight: 600, paddingLeft: '0.5rem' }}>MENU</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                        <a
                            href="/"
                            target="_blank"
                            rel="noopener noreferrer"
                            style={{ textAlign: 'left', padding: '0.5rem 0.75rem', color: '#22d3ee', background: 'rgba(34,211,238,0.1)', border: 'none', cursor: 'pointer', fontSize: '0.9rem', borderRadius: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem', textDecoration: 'none' }}
                        >
                            <span>🌐</span> Visiter le site
                        </a>
                        <button
                            onClick={() => router.push('/admin/leads')}
                            style={{ textAlign: 'left', padding: '0.5rem 0.75rem', color: '#9ca3af', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '0.9rem', borderRadius: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}
                        >
                            <span>👥</span> Leads
                        </button>
                        <button
                ...
                <div style={{ flex: 1, padding: '0 1rem', overflowY: 'auto' }}>
                    <div style={{ fontSize: '0.7rem', color: '#6b7280', marginBottom: '0.75rem', fontWeight: 600, paddingLeft: '0.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span>PAGES</span>
                        <button
                            onClick={handleAddPage}
                            style={{ background: '#22d3ee', border: 'none', borderRadius: '0.25rem', color: '#000', width: '1.25rem', height: '1.25rem', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                        {pages.map(page => (
                            ...
                            >
                                <span style={{ color: selectedPageId === page.id ? '#22d3ee' : '#9ca3af', fontSize: '0.9rem' }}>
                                    {page.name}
                                </span>
                                {page.slug !== 'home' && (
                                    <button
                                        onClick={(e) => {
                                            ...
                                            if (confirm(`Supprimer la page "${page.name}" ?`)) {
                                                deletePage(page.id).then(() => loadPages());
                                            }
                                        }}
                                        style={{ background: 'transparent', border: 'none', color: '#ef4444', cursor: 'pointer', padding: '0.25rem', fontSize: '0.75rem', opacity: 0.6 }}
                                        title="Supprimer"
                                    >
                                        ×
                                    </button>
