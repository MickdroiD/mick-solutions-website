...
import { ArrowLeft, Save, Loader2, Check, Image, Sparkles, Upload } from 'lucide-react';
import AssetPicker from '@/features/admin/components/AssetPicker';
...
export default function SettingsClientPage({ tenantId }: SettingsPageProps) {
    const router = useRouter();
    const { data: session } = useSession(); // Session still useful for user info but tenantId comes from server
    ...
    const [assetPickerField, setAssetPickerField] = useState<keyof SiteConfig | null>(null);
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#0a0a0f', color: '#fff' }}>
                ...
                {activeTab === 'header' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        ...
                        <ImageField
                            ...
                            onPickerOpen={() => { setAssetPickerField('headerLogoUrl'); setAssetPickerOpen(true); }}
                            onClear={() => updateField('headerLogoUrl', '')}
                        />
                        <Toggle
                ...
                {activeTab === 'advanced' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        ...
                        <div style={{
                            ...
                            <ImageField
                                label="Image source pour le favicon"
                                value={logoUrl}
                                onPickerOpen={() => setFaviconPickerOpen(true)}
                                onClear={() => setLogoUrl('')}
                            />
                            <div style={{ marginTop: '1rem' }}>
                                ...
                                >
                                    {faviconGenerating ? (
                                        ...
                                        <><Image size={18} /> Générer les Favicons</>
                                    )}
                                </button>
                            </div>
            ...
            <AssetPicker
                ...
                currentUrl={assetPickerField && config ? (config[assetPickerField] as string) || '' : ''}
                onSelect={(url) => {
                    if (assetPickerField) {
                        updateField(assetPickerField, url);
                    }
